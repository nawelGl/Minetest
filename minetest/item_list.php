...
require_once __DIR__.'/class/item.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('minetest', 'item', 'write');
	$permissiontodelete = $user->hasRight('minetest', 'item', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
//$help_url = "EN:Module_Item|FR:Module_Item_FR|ES:Módulo_Item";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && !getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/minetest/item_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/minetest/item_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendItemRef";
$modelmail = "item";
$objecttmp = new Item($db);
