...
/**
 *   	\file       joueur_list.php
 *		\ingroup    minetest
 *		\brief      List page for joueur
 */
...
require_once __DIR__.'/class/joueur.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('minetest', 'joueur', 'write');
	$permissiontodelete = $user->hasRight('minetest', 'joueur', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("minetest")) {
	accessforbidden('Module minetest not enabled');
}
...
if (empty($reshook)) {
	...
	$objectclass = 'Joueur';
	$objectlabel = 'Joueur';
	$uploaddir = $conf->minetest->dir_output;
...
$title = $langs->trans("Joueurs");
//$help_url = "EN:Module_Joueur|FR:Module_Joueur_FR|ES:Módulo_Joueur";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && !getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/minetest/joueur_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/minetest/joueur_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendJoueurRef";
$modelmail = "joueur";
$objecttmp = new Joueur($db);
